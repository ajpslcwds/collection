...
#include <stdarg.h>
#include <time.h>
#if (defined(_MSC_VER) && _MSC_VER >= 1600) || (defined(__BORLANDC__) && __BORLANDC__ >= 0x0630) ||                    \
...
typedef enum eSALibraryErrorType
{
...
typedef enum eSAIsolationLevel
{
...
typedef enum eSALongOrLobReaderModes
{
...
typedef size_t(SQLAPI_CALLBACK *saLongOrLobWriter_t)(SAPieceType_t &ePieceType, void *pBuf, size_t nLen,
                                                     void *pAddlData);
...
typedef void(SQLAPI_CALLBACK *saLongOrLobReader_t)(
    SAPieceType_t ePieceType, //!< Passed to the function from SQLAPI++ Library mechanisms. It  defines which piece of
                              //!< data value was read.
    ...
    size_t nBlobSize, //!< Maximum size of whole long or Lob field (if DBMS server allows to know it before whole field
...
class SQLAPI_API SAInterval
{
  public:
    ...
    SAInterval(int nDays, int nHours, int nMins,
               int nSecs); //!< Initializes with defined days, hours, minutes and seconds.
    SAInterval(int nDays, int nHours, int nMins, int nSecs,
               unsigned int nNanoSeconds); //!< Initializes with defined days, hours, minutes, seconds and nanoseconds
...
class SQLAPI_API SADateTime
{
  ...
  protected:
    static bool DateFromTm(unsigned short wYear, unsigned short wMonth, unsigned short wDay, unsigned short wHour,
                           unsigned short wMinute, unsigned short wSecond, unsigned int nNanoSecond, bool hasDate,
                           double &dtDest);
    static bool TmFromDate(double dtSrc, struct tm &tmDest, unsigned int &nNanoSecond);

  protected:
    void Init_Tm();
    void Init(int nYear, int nMonth, int nDay, int nHour, int nMin, int nSec, unsigned int nFraction,
              const SAChar *timezone);
    struct tm m_tm;
  ...
  public:
    ...
    //! Returns a reference to the current value of SADateTime object as struct tm object. You can modify this structure
    //! directly.
    operator struct tm &();
...
*/
typedef enum eSAConnectionHandlerType
{
...
class SQLAPI_API SACommand : public SAOptions
{
  ...
  private:
    ...
    static int CompareIdentifier(const SAString &sIdentifier1, const SAString &sIdentifier2);
    SAParam &CreateParam(const SAString &sName, SADataType_t eParamType, int nNativeType, size_t nParamSize,
                         int nParamPrecision, int nParamScale, SAParamDirType_t eDirType, const SAString &sFullName,
                         size_t nStart, // param position in SQL statement
                         size_t nEnd);  // param end position in SQL statement
    void GetParamsSP();
    void UnDescribeParams();
    void ParseInputMarkers(SAString &sCmd, bool *pbSpacesInCmd);

    void DescribeFields() SQLAPI_THROW(SAException);
    void CreateField(const SAString &sName, SADataType_t eFieldType, int nNativeType, size_t nFieldSize,
                     int nFieldPrecision, int nFieldScale, bool bFieldRequired, int nTotalFieldCount);
    void DestroyFields();

    // parses sql statement and create bind parameters array if any (In)
    // also cancels previous statement if any
    void ParseCmd(const SAString &sSQL, SACommandType_t eCmdType);
  ...
  public:
    ...
    void setCommandText(const SAString &sSQL, //!< A SAString object represents command text string (an SQL statement or
    ...
    SAParam &CreateParam(const SAString &sName,                    //!< A string representing the name of parameter
...
class SQLAPI_API SAValueRead
{
  ...
  protected:
    ...
    size_t PrepareReader(size_t nExpectedSizeMax, // to optimize buf allocation for internal buffer, 0 = unknown
                         size_t nCallerMaxSize,   // max Piece that can be processed by API
                         unsigned char *&pBuf, saLongOrLobReader_t fnReader, size_t nReaderWantedPieceSize,
                         void *pReaderAddlData, bool bAddSpaceForNull = false);
    void InvokeReader(SAPieceType_t ePieceType, unsigned char *&pBuf, size_t nPieceLen);
...
class SQLAPI_API SAValue : public SAValueRead
{
  ...
  protected:
    ...
    size_t InvokeWriter(SAPieceType_t &ePieceType, size_t nCallerMaxSize, void *&pBuf);
...
class SQLAPI_API SAParam : public SAValue, public SAOptions
{
  ...
  protected:
    SAParam(SACommand *pCommand, const SAString &sName, SADataType_t eParamType, int nNativeType, size_t nParamSize,
            int nParamPrecision, int nParamScale, SAParamDirType_t eDirType);
    virtual ~SAParam();
  ...
  public:
    ...
    void ReadLongOrLob(saLongOrLobReader_t fnReader, size_t nReaderWantedSize,
...
class SQLAPI_API SALongOrLob : public SAString
{
  ...
  protected:
    ...
    SALongOrLob(saLongOrLobWriter_t fnWriter, size_t nWriterPieceSize, void *pAddlData);
};
...
class SQLAPI_API SALongBinary : public SALongOrLob
{
  public:
    ...
    SALongBinary(saLongOrLobWriter_t fnWriter, size_t nWriterPieceSize, void *pAddlData);
};
...
class SQLAPI_API SALongChar : public SALongOrLob
{
  public:
    ...
    SALongChar(saLongOrLobWriter_t fnWriter, size_t nWriterPieceSize, void *pAddlData);
};
...
class SQLAPI_API SABLob : public SALongOrLob
{
  public:
    ...
    SABLob(saLongOrLobWriter_t fnWriter, size_t nWriterPieceSize, void *pAddlData);
};
...
class SQLAPI_API SACLob : public SALongOrLob
{
  public:
    ...
    SACLob(saLongOrLobWriter_t fnWriter, unsigned int nWriterPieceSize, void *pAddlData);
};
...
class SQLAPI_API SAField : public SAValueRead, public SAOptions
{
  ...
  protected:
    SAField(SACommand *pCommand,
            int nPos, // 1-based
            const SAString &sName, SADataType_t eFieldType, int nNativeType, size_t nFieldSize, int nFieldPrecision,
            int nFieldScale, bool bFieldRequired);
    virtual ~SAField();
...
#endif
{
  ...
  public:
    SAException(SAException *pNested, SAErrorClass_t eError, int nNativeError, int nErrPos, const SAString &sMsg);
    SAException(SAException *pNested, SAErrorClass_t eError, int nNativeError, const SAString &sCommandText,
                int nErrPos, const SAString &sMsg);
    SAException(SAException *pNested, SAErrorClass_t eError, int nNativeError, int nErrPos, const SAChar *lpszFormat,
                ...);

    SAException(SAErrorClass_t eError, int nNativeError, int nErrPos, const SAString &sMsg);
    SAException(SAErrorClass_t eError, int nNativeError, const SAString &sCommandText, int nErrPos,
                const SAString &sMsg);
    SAException(SAErrorClass_t eError, int nNativeError, int nErrPos, const SAChar *lpszFormat, ...);
    SAException(SAErrorClass_t eError, int nNativeError, const SAString &sCommandText, int nErrPos,
...
\see SAGlobals::SetTraceFunction
*/
typedef void(SQLAPI_CALLBACK *SATraceFunction_t)(SATraceInfo_t traceInfo,   //!< Tracing into label.
...
class SQLAPI_API SAGlobals
{
  public:
    ...
    static SAClient_t StringToClient(
        const SAChar *szClientName); //!< Returns a DBMS client type of a specified client name.
...
typedef struct SADateTime SADateTime;
    ...
    SQLAPI_API SAErrorClass_t sqlapi_connect(SAConnection *conn, const SAChar *szConnectionStr, const SAChar *szUser,
                                             const SAChar *szPassword, SAException *err);
    ...
    SQLAPI_API SAErrorClass_t sqlapi_set_isolation_level(SAConnection *conn, SAIsolationLevel_t level,
                                                         SAException *err);
    ...
    SQLAPI_API const SAValueRead *sqlapi_param_value_by_name(SACommand *cmd, const SAChar *szParamByName,
                                                             SAException *err);
    ...
    SQLAPI_API void sqlapi_datetime_set_datetime(SADateTime *dt, int nYear, int nMonth, int nDay, int nHour, int nMin,
