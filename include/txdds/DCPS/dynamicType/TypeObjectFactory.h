 ...
 * @FilePath     : /TXDDS/include/DCPS/dynamicType/TypeObjectFactory.h
...
namespace BaoSky::dds
{
    ...
    public:
        TypeObjectFactory();
        mutable std::map<std::string, TypeIdentifier *> identifiers_;  // Basic, builtin and EK_MINIMAL
        std::map<std::string, TypeIdentifier *> complete_identifiers_; // Only EK_COMPLETE
        std::map<TypeIdentifier *, TypeObject *> objects_;             // EK_MINIMAL
        std::map<TypeIdentifier *, TypeObject *> complete_objects_;    // EK_COMPLETE
    ...
    public:
        ...
        void add_type_object(const std::string &type_name,
        ...
        std::shared_ptr<BaoSky::dds::DynamicType> BuildDynamicType(const std::string &name,
                                                                   const TypeIdentifier *identifier,
                                                                   const TypeObject *object);

        std::shared_ptr<BaoSky::dds::DynamicType> BuildDynamicType(
            TypeDescriptor &descriptor,
            const TypeObject *object,
            const DynamicType *annotation_member_type);
        TypeIdentifier *CreateComplexType(const TypeIdentifier *identifier);
        TypeIdentifier *get_array_identifier(
            const std::string &type_name,
            const std::vector<uint32_t> &bound);

        inline bool add_type_identifier(
            const std::string &type_name,
            const TypeIdentifier *identifier)
        {
            std::unique_lock<std::mutex> lck(mMutex);
            if (identifier->m__d == EK_COMPLETE)
            {
        ...
        std::string get_array_type_name(
            const std::string &type_name,
            const std::vector<uint32_t> &bound,
            uint32_t &ret_size);

        TypeIdentifier *get_sequence_identifier(
            const std::string &type_name,
            uint32_t bound);

        std::string get_sequence_type_name(
            const std::string &type_name,
            uint32_t bound);

        TypeIdentifier *get_map_identifier(
            const std::string &key_type_name,
            const std::string &value_type_name,
            uint32_t bound);

        std::string get_map_type_name(
            const std::string &key_type_name,
            const std::string &value_type_name,
            uint32_t bound);
        std::string get_string_type_name(
            uint32_t bound,
            bool wide);

        TypeIdentifier *get_string_identifier(
            uint32_t bound,
            bool wide);
    };
