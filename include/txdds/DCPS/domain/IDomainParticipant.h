...
namespace BaoSky::dds
{
    ...
    class IDomainParticipant : public IEntity
    {
    public:
        ...
        virtual IPublisher *CreatePublisher(
            const PublisherQos &qos,
            IPublisherListener *listener = nullptr,
            const StatusMask &mask = ALL_STATUS) = 0;

        virtual ReturnCode DeletePublisher(IPublisher *publisher) = 0;

        virtual ISubscriber *CreateSubscriber(
            const SubscriberQos &qos,
            ISubscriberListener *listener = nullptr,
            const StatusMask &mask = ALL_STATUS) = 0;
        ...
        virtual ITopic *CreateTopic(
            const std::string &topicName,
            const std::string &typeName,
            const TopicQos &qos,
            ITopicListener *listener = nullptr,
            const StatusMask &mask = ALL_STATUS) = 0;
