...
namespace BaoSky::rtps
{
    struct PoolConfig
    {
        ...
        constexpr PoolConfig(
            ...
            : mMemoryPolicy(policy),
              mPayloadInitialSize(payloadSize),
              mInitialSize(iniSize), mMaximumSize(maxSize)
        {
        }
        ...
        static constexpr PoolConfig FromHistoryAttributes(
            const HistoryAttributes &historyAttr) noexcept
        {
            // todo pool的数量限制由什么决定
            return {
                ...
                static_cast<uint32_t>(
                    historyAttr.mInitialReservedCaches <= 0 ? 0 : historyAttr.mInitialReservedCaches + historyAttr.mExtraReservedCaches + 500),
                static_cast<uint32_t>(
                    historyAttr.mMaximumReservedCaches <= 0 ? 0 : historyAttr.mMaximumReservedCaches + historyAttr.mExtraReservedCaches + 500)};
        };
    };
}
