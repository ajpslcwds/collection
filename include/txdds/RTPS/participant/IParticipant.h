...
namespace BaoSky::rtps
{
    ...
    {
    public:
        IParticipant(const uint32_t &domainId, const GUID &guid) : IEntity(guid), mDomainId(domainId) {}

        virtual ~IParticipant() override {}

        virtual IWriter *CreateWriter(
            IWriterHistory *history,
            WriterAttributes &watt,
            const bool &isBuiltin,
            const std::shared_ptr<IPayloadPool> &payloadPool,
            const std::shared_ptr<IChangePool> &changePool,
            IWriterListener *listener = nullptr,
            const EntityId &entityId = ENTITYID_UNKNOWN) = 0;

        virtual IReader *CreateReader(
            IReaderHistory *history,
