...
{
    using octet = uint8_t;
    typedef std::array<uint32_t, 8> LongSeq8;
    enum Endianness_t
    {
        BIGEND = 0x1,
        LITTLEEND = 0x0
...
#else
    constexpr Endianness_t DEFAULT_ENDIAN = BIG_ENDIAN;
#endif
    // ProtocolId_t
    enum ProtocolId_t
    {
    ...
    typedef octet Checksum128_t[32];
    // Checksum32_t or Checksum64_t or Checksum128_t
    typedef octet *Checksum_t;
    const Checksum_t CHECKSUN_INVALID = nullptr;
    // MessageLength_t
