...
namespace BaoSky::rtps
{
    ...
    public:
        HistoryAttributes(
            uint32_t payloadMaxSize = 500,
            uint32_t initialReservedCaches = 20,
            uint32_t maximumReservedCaches = 100,
            uint32_t extraReservedCaches = 1,
            MemoryManagementPolicy_t memoryPolicy = PREALLOCATED_WITH_REALLOC_MEMORY_MODE)
            : mInitialReservedCaches(initialReservedCaches),
              mMaximumReservedCaches(maximumReservedCaches),
              mExtraReservedCaches(extraReservedCaches),
              mPayloadMaxSize(payloadMaxSize),
