...
namespace BaoSky::rtps
{
    ...
    class IReader : public IEndpoint
    {
    public:
        IReader(
            const GUID &guid,
        ...
        virtual bool RemoveMatchedWriter(const GUID &guid) = 0;

        virtual bool IsMatchedWriter(const GUID &guid) = 0;
        ...
        virtual void ProcessGapMsg(
            const GUID &writerGUID,
            const SequenceNumber &gapStart,
            const SequenceNumberSet &gapList,
            const ChangeCount &filterCount) = 0;
        ...
        virtual void update_last_notified(const GUID &guid, const SequenceNumber &seq);
