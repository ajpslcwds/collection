...
template <typename T> int test_read(const std::string &redis_key)
{
    ...
    std::cout << "value:" << value[2] << ",time_s:" << time_s << ",time_ms:" << time_ms << ",quality:" << quality
              << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    auto tNextFireTime = std::chrono::steady_clock::now();
    while (true)
    {
        std::this_thread::sleep_until(tNextFireTime);
        tNextFireTime += std::chrono::milliseconds(50);
        test_read<int16_t>(tag_name);
    }
